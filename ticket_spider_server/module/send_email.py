...
def send_email(email_content=' ', db_path_1='', db_path_2='', log_path='', send_to_addr='wangcser@qq.com'):
    '''
    name: 发送邮件模块
    ...
    mime_text = MIMEText(header_text + email_content, 'plain', 'utf-8') # 实例化一个文本邮件对象
    msg.attach(mime_text)

    # 添加邮件附件信息: db 1
    with open(db_path_1,'rb') as f:
        # 设置附件的MIME和文件名
        dir_path = os.path.dirname(db_path_1)
        file_name = db_path_1[len(dir_path):] # 获取文件名
    ...
    # 添加邮件附件信息: db 2
    with open(db_path_2,'rb') as f:
        # 设置附件的MIME和文件名
        dir_path = os.path.dirname(db_path_2)
        file_name = db_path_2[len(dir_path):] # 获取文件名
        mime = MIMEBase('database', 'db' , filename = file_name)
        # 加上必要的头部信息
        mime.add_header('Content-Disposition', 'attachment', filename=file_name)
        mime.add_header('Content-ID', '<0>')
...
def send_log_email(email_content=' ', log_path='', send_to_addr='wangcser@qq.com'):
    '''
    name: 发送邮件模块
    func: 调用该模块向指定地址发送邮件，已设定默认正文，默认收件人和默认附件，目前不考虑接受log信息（本地调试）
    send_email(attch_text='', file_path='', send_to_addr='')
